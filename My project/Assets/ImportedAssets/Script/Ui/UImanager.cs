...
public class UImanager : MonoBehaviour {
    public GameObject showslidermaxspeed;
...
//            initadmod.instance.ShowBanerOnPlayGame();
    ...
    IEnumerator Loadingonthestart()
    {
         ...
         for (int i = 0; i <100; i++)
        {
            ...
            if (i== 99)
            {
                ...
                awakemainmenu();
                ManagernewSceen.alowmuving = true;
//                initadmod.instance.ShowFullNowOnOpenGame();
            }
        }
    }
    ...
    public void play()
    {
        ...
        panelmain.SetActive(false);
        showslidermaxspeed.SetActive(true);
        StartCoroutine(Playdelay());
        Soundmanager.soundmanager.PlayXe();
        ManagernewSceen.startmuvingbus = true;
    ...
    IEnumerator Playdelay()
    {
       ...
       // Debug.Log("chơi lại");
        ...
        folow.distance = 10;
        yield return new WaitForSeconds(0.3f);
        ManagernewSceen.Intance.TaiNan();
...
//        initadmod.instance.ShowBanerOnPlayGame();
    ...
    public void Lost()
    {
        ...
        if (alowcall)
        {
            ...
            Slideraxspeed.instance.Getfalse();
        }
    }
     public   int showbane = 0;
    ...
    public Image _imgslidercicle;
    public GameObject BtnPause;
    bool _continuedMuvingWenCliCads = false;
    public Text showdelay;
    int delay = 5;
    int alowshowads = 0;
    /// <summary>
    ///     
    ...
    public IEnumerator delayforAgain()
    {
        ...
        delay = 5;
        showdelay.text = delay.ToString();
        _imgslidercicle.GetComponent<Image>().fillAmount = 1;
        // _btnunityAds.SetActive(UnityAds.instance.UnityAdsloaded());
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            _imgslidercicle.GetComponent<Image>().fillAmount -= 0.01f;
            if (i>0&&i%20==0)
            {
                delay -= 1;
                showdelay.text = delay.ToString();
...
//                initadmod.instance.HideBaner();
                inthepanelpause.playagain = true;
                if (showbane >= 2)
                {
                    //  GoogleMobileAdsScript.Instance.showfullbaner();
//                    initadmod.instance.ShowFullOnDie();
                   // showbane = 0;
                }
                 if (selectkey == false)
                    {
    ...
    public GameObject _btnunityAds;
    //public void GetUnityAds()
    //{
    //    if (_btnunityAds.transform.GetChild(1).gameObject.active)
    //    {
    //        if (Advertisement.IsReady())
    //        {
    //            _continuedMuvingWenCliCads = true;
    //        }
  ...
  public void Playnow()
    {
        ...
        yourkey.text = managerdata.manager.getkey().ToString();
        if (_continuedMuvingWenCliCads==false)
        {
            managerdata.manager.savekey(-needkey);
        }
        emty.emtyplayer.ResutTranformemty();
        Perencamera.managerscen.height = 3;
        _continuedMuvingWenCliCads = false;
...
//        initadmod.instance.HideBaner();
        //  GoogleMobileAdsScript.Instance.showfullbaner();
//        initadmod.instance.ShowFullOnBackToMainMenu();
        Time.timeScale = 1;
        IkEmty.iklegth1 = 0;
        IkEmty.iklegth = 0;
    ...
    *****************************************/
    ...
    void Update () {
      ...
      //  Debug.Log(Playermuving.speedmuving);
        if (Playermuving.player != null)
        {
            if (Playermuving.isplay)
            {
                ...
                if (Time.timeScale <= 1.1f)
                {
                    ...
                    {
                       // Debug.Log("đi" + coinmuving);
                      //  Debug.Log("chốt " + coinforuplv);
                        coinforuplv = coinforuplv * 2;
                        Time.timeScale += 0.05f;
                        Playermuving.speedmuving += 1.5f;
                    }
                }
            }
    ...
    int gettexint = 0;
    float time;
  public  static  int coinforuplv = 2000;
